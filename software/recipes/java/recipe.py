"""JavaRecipe 主类：纯调度，零平台 if，所有平台差异通过 PlatformDriver 隔离"""
...
class JavaRecipe(Recipe):
    ...
    def installed_versions(self) -> list[str]:
        ...
        for entry in base.iterdir():
            ...
            if not name.startswith("jdk"):
                continue
            ver = name[3:].replace("_", "+")   # jdk21.0.11_10 → 21.0.11+10
            if ver and ver[0].isdigit():
                versions.append(ver)
        versions.sort(
            key=lambda v: [int(x) for x in v.split("+")[0].split(".") if x.isdigit()],
            reverse=True,
        )
        return versions
    ...
    def install(self, version: str) -> None:
        ...
        with MultiStepProgress(descs) as sp:
            ...
            with tempfile.TemporaryDirectory(prefix="opskit-java-") as tmpdir:
                ...
                t_pct.start()
                try:
                    download_java_tarball(version, tarball)
                finally:
                    _stop_pct.set()
            ...
            if not self.detect():
                raise InstallError(t("software.java_error.verify_failed"))
            sp.complete()
    ...
    def switch(self, version: str) -> None:
        ...
        if version not in installed:
            raise InstallError(t("software.java_error.not_installed", version=version))

        bin_dir = str(java_bin_dir(version))
        snap = load_snapshot()
        snap["active_version"] = version
        snap["java_bin_dir"] = bin_dir
        save_snapshot(snap)
