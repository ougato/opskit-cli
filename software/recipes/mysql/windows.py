"""MySQL Windows 平台驱动：zip 解压、shim cmd、注册表 PATH 注入（对齐 mongodb/windows.py）"""
...
from .common import detect_mysql_version
from .constants import (
    WIN_MYSQL_PS_MARKER,
    WIN_MYSQL_PS_END,
    WIN_MYSQL_CMD_MARKER,
    WIN_MYSQL_CMD_END,
    CMD_AUTORUN_MYSQL_SUBPATH,
...
class WindowsDriver(PlatformDriver):
    ...
    def install_tarball(self, version: str, tarball: Path) -> str:
        """
        将 zip 解压到版本专属目录：~/.opskit/mysql/mysql{version}/
        返回 bin 目录路径字符串。
        """
        ...
        if not (bin_dir / "mysql.exe").exists():
            raise InstallError(t("software.mysql_error.win_bad_structure", version=version))
        return str(bin_dir)
    ...
    def install_shim(self, fallback_bin: str) -> None:
        ...
        for name in ("mysql.cmd", "mysqld.cmd", "mysqladmin.cmd", "mysqldump.cmd"):
            (sdir / name).write_text(SHIM_MYSQL_CMD_TEMPLATE, encoding="utf-8")
    ...
    def _active_junction(self) -> Path:
        from .common import mysql_versions_dir
        return mysql_versions_dir() / "active"
    ...
    def apply_version_link(self, bin_dir: str) -> None:
        """
        切换时更新 junction ~/.opskit/mysql/active -> 激活版本目录。
        ...
        if junction_ok:
            ...
            for stem, exe in (("mysql", "mysql.exe"), ("mysqld", "mysqld.exe"),
                               ("mysqladmin", "mysqladmin.exe"), ("mysqldump", "mysqldump.exe")):
                src = bin_path / exe
                if not src.exists():
                    continue
    ...
    def detect_active(self) -> str | None:
        return detect_mysql_version("mysql.exe")
    ...
    def _inject_path(self, path_to_add: str) -> None:
        ...
        for ps_profile in (
            ...
            try:
                ...
                if WIN_MYSQL_PS_MARKER not in text:
                    ps_profile.write_text(text + ps_block, encoding="utf-8")
            except Exception:
                pass

        bat_path = str(Path.home() / CMD_AUTORUN_MYSQL_SUBPATH)
        bat_content = (
            f"@echo off\r\n"
            f"@{WIN_MYSQL_CMD_MARKER}\r\n"
    ...
    def _remove_path(self, path_to_remove: str) -> None:
        ...
        for ps_profile in (
            ...
            try:
                ...
                out, skip = [], False
                for line in lines:
                    if line.strip() == WIN_MYSQL_PS_MARKER:
                        skip = True
                    if not skip:
                        out.append(line)
                    if line.strip() == WIN_MYSQL_PS_END:
                        skip = False
