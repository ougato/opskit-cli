...
opskit -V                             # 同上（短参数）
opskit --theme catppuccin             # 启动时指定主题（短参数 -T）
opskit --lang zh                      # 启动时指定语言（zh / en，短参数 -l）
opskit --theme catppuccin --lang en   # 同时指定主题和语言
```

> **参数大小写不敏感**：所有命令名、长短选项均大小写等价，例如 `-V` 与 `-v`、`-h` 与 `-H`、`--help` 与 `--HELP`、`software` 与 `SOFTWARE` 均可。每个长选项都补齐了单字符短别名（`--help/-h`、`--version/-V`、`--theme/-T`、`--lang/-l`、`--yes/-y`、`--token/-t`、`--all/-a`、`--version/-v`）。
...
```bash
opskit software diagnose tailscale    # Tailscale 组网状态诊断
opskit software diagnose xui          # x-ui 面板诊断
opskit software diagnose rustdesk     # RustDesk 服务诊断
```

> 支持诊断的软件：`wg_server` / `wg_client` / `tailscale` / `xui` / `rustdesk`，其他软件的诊断菜单项不可用。
...
```bash
opskit software manage tailscale      # Tailscale 管理
opskit software manage xui            # x-ui 面板管理
```

> 支持管理的软件：`wg_server` / `wg_client` / `tailscale` / `xui`，其他软件的管理菜单项不可用。
...
update:
  enabled: true       # 启用自动更新（每次启动后台静默检测，退出/下次启动无缝切换）
log:
